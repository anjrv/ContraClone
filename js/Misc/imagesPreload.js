...
Image.prototype.asyncLoad = function (src, asyncCallback) {
  ...
  console.debug('requesting image src of ', src);
  this.src = src;
};
...
function imagesPreload(requiredImages, loadedImages, completionCallback) {
  ...
  preloadHandler = function () {
    console.debug('preloadHandler called with this=', this);
    loadedImages[this.name] = this;

    if (0 === this.width) {
      console.debug('loading failed for', this.name);
    }
    ...
    if (numImagesHandled === numImagesRequired) {
      console.debug('all preload images handled');
      console.debug('loadedImages=', loadedImages);
      console.debug('');
      console.debug('performing completion callback');

      completionCallback();

      console.debug('completion callback done');
      console.debug('');
    }
  };
  ...
  for (currentName in requiredImages) {
    ...
    if (requiredImages.hasOwnProperty(currentName)) {
      console.debug('preloading image', currentName);
